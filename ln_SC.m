...
for i = 1:N
    for k = 1:N
        ...
        if i/N > vd(k,2)%4 for sobol, 7 for vdc, 8 for LFSR --4
        %if i/N > sobol(k,2)
            X2_stream_vdc(i,k) = 1;
        end
...
%     end
    ...
    ln_sobol(i) = sum(y_s(i,:))/N;
    %abs_sin_sob(i) = abs(sin_sobol(i) - EE(i));
    abs_ln_sob(i) = (ln_sobol(i) - EE(i))^2;

    input = X2_stream_sobol(i,:);
    n1_v(i,:) = not(and(input, circshift(X2_stream_vdc(ceil((4/5)*N),:),0)));%best with 15 Delays
    %n1_v(i,:) = and(X4_stream_vdc(i,:), circshift(X4_stream_vdc(i,:),3));
    n2_v(i,:) = not(and3(n1_v(i,:), circshift(X3_stream_vdc(ceil((3/4)*N),:),0), circshift(input,0)));
    n3_v(i,:) = not(and3(circshift(X5_stream_vdc(ceil((2/3)*N),:),0), n2_v(i,:), circshift(input,0)));
    n4_v(i,:) = not(and3(circshift(X4_stream_vdc(ceil((1/2)*N),:),0), n3_v(i,:), circshift(input,0)));
...
%     y_lf(i,:) = and(n4_lf(i,:), circshift(X2_stream_lfsr(i,:),6));
    n1_lf(i,:) = not(and(X2_stream_lfsr_input(i,:), circshift(X2_stream_lfsr_coeff(uint32((4/5)*N),:),0)));
    n2_lf(i,:) = not(and3(n1_lf(i,:), circshift(X2_stream_lfsr_coeff(uint32((3/4)*N),:),0), circshift(X2_stream_lfsr_input(i,:),1)));
    n3_lf(i,:) = not(and3(circshift(X2_stream_lfsr_coeff(uint32((2/3)*N),:),0), n2_lf(i,:), circshift(X2_stream_lfsr_input(i,:),2)));
    n4_lf(i,:) = not(and3(circshift(X2_stream_lfsr_coeff(uint32((1/2)*N),:),1), n3_lf(i,:), circshift(X2_stream_lfsr_input(i,:),3)));
    y_lf(i,:) = and(n4_lf(i,:), circshift(X2_stream_lfsr_input(i,:),4));
