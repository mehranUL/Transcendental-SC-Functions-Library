...
for i = 1:N
    ...
    n1_v(i,:) = and(input, circshift(input,2));
    n2_v(i,:) = not(and(n1_v(i,:), X2_stream_vdc(ceil((1/10)*N),:)));
    n3_v(i,:) = not(and3(X3_stream_vdc(ceil((1/12)*N),:), n2_v(i,:), circshift(n1_v(i,:),0)));
    n4_v(i,:) = not(and3(X3_stream_vdc(ceil((1/2)*N),:), n3_v(i,:), circshift(input,0)));
    y_v(i,:) = not(and(n4_v(i,:), X4_stream_vdc(ceil((1/2)*N),:)));
    ...
    n2_lf(i,:) = not(and(n1_lf(i,:), X2_stream_lfsr_coeff(ceil((1/10)*N),:)));
    n3_lf(i,:) = not(and3(circshift(X2_stream_lfsr_coeff(ceil((1/12)*N),:), 1), n2_lf(i,:), circshift(n1_lf(i,:),1)));
    n4_lf(i,:) = not(and3(circshift(X2_stream_lfsr_coeff(ceil((1/2)*N),:), 1), n3_lf(i,:), circshift(X2_stream_lfsr(i,:),4)));
    y_lf(i,:) = not(and(n4_lf(i,:), circshift(X2_stream_lfsr_coeff(ceil((1/2)*N),:),0)));
...
MSE_lfsr(jj) = mean(abs_sigmoid_lf);
end
