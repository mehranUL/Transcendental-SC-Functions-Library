...
for i = 1:N
    for k = 1:N
        ...
        if i/N > vd(k,4)%5 for sobol, 10 for vdc, 1 for LFSR
        %if i/N > z1(k)
            X2_stream_vdc(i,k) = 1;
        end
        if i/N > vd(k,6)%6 %5,4
...
for i = 1:N
    ...
    n2_s(i,:) = not(and(n1_s(i,:), X2_stream_sobol(ceil((1/5040)*N),:)));
    n3_s(i,:) = and(n2_s(i,:), X2_stream_sobol(ceil((1/120)*N),:));
    n4_s(i,:) = not(and(n3_s(i,:), circshift(n1_s(i,:), 1)));%1
    n5_s(i,:) = and(n4_s(i,:), X2_stream_sobol(ceil((1/6)*N),:));
    ...
    abs_sin_vd(i) = (sin_vdc(i) - EE(i))^2;

    n1_lf(i,:) = and(X2_stream_lfsr_input(i,:), circshift(X2_stream_lfsr_input(i,:),1));
    n2_lf(i,:) = not(and(n1_lf(i,:), circshift(X2_stream_lfsr_coeff(ceil((1/5040)*N),:),0)));
    n3_lf(i,:) = and(n2_lf(i,:), circshift(X2_stream_lfsr_coeff(ceil((1/120)*N),:),0));
    n4_lf(i,:) = not(and(n3_lf(i,:), circshift(n1_lf(i,:), 1)));%1
    n5_lf(i,:) = and(n4_lf(i,:), circshift(X2_stream_lfsr_coeff(ceil((1/6)*N),:),0));
    ...
    abs_sin_lf(i) = (sin_lfsr(i) - EE(i))^2;
end
%MSE_sobol = mean(abs_sin_sob)
%MSE_vdc = mean(abs_sin_vd)
MSE_lfsr(jj) = mean(abs_sin_lf);
end
