...
%     end
    ...
    n1_v_sin(i,:) = and(input, circshift(input,3));%best with 15 Delays
    %n1_v(i,:) = and(X4_stream_vdc(i,:), circshift(X4_stream_vdc(i,:),3));
    n2_v_sin(i,:) = not(and(n1_v_sin(i,:), X2_stream_vdc_sin(ceil((1/42)*N),:)));
    n3_v_sin(i,:) = not(and3(X3_stream_vdc_sin(ceil((1/20)*N),:), n2_v_sin(i,:), circshift(n1_v_sin(i,:),1)));
    n4_v_sin(i,:) = not(and3(X4_stream_vdc_sin(ceil((1/6)*N),:), n3_v_sin(i,:), circshift(n1_v_sin(i,:),0)));%0
    y_v_sin(i,:) = and(n4_v_sin(i,:), circshift(input,1));%1
    ...
    %input = X2_stream_vdc(i,:);
    %input = X2_stream_lfsr_input(i,:);
    n1_v_cos(i,:) = and(input, circshift(input,3));%best with 15 Delays
    n2_v_cos(i,:) = not(and(n1_v_cos(i,:), X2_stream_vdc_cos(ceil((1/56)*N),:)));
    %n2_v_cos(i,:) = n1_v_cos(i,:);
    n3_v_cos(i,:) = not(and3(X3_stream_vdc_cos(ceil((1/30)*N),:), n2_v_cos(i,:), circshift(n1_v_cos(i,:),0)));%0
    n4_v_cos(i,:) = not(and3(X4_stream_vdc_cos(ceil((1/12)*N),:), n3_v_cos(i,:), circshift(n1_v_cos(i,:),0)));%0
    y_v_cos(i,:) = not(and3(X5_stream_vdc_cos(ceil((1/2)*N),:), n4_v_cos(i,:), circshift(n1_v_cos(i,:),1)));%1
    cos_vdc(i) = sum(y_v_cos(i,:))/N;

%----------------------------------Conventional----------------------------------------------------------------    
    n1_lf_sin(i,:) = and(X2_stream_lfsr_input(i,:), circshift(X2_stream_lfsr_input(i,:),3));
    n2_lf_sin(i,:) = not(and(n1_lf_sin(i,:), circshift(X2_stream_lfsr_coeff(ceil((1/42)*N),:),1)));
    n3_lf_sin(i,:) = not(and3(circshift(X2_stream_lfsr_coeff(ceil((1/20)*N),:),1), n2_lf_sin(i,:), circshift(n1_lf_sin(i,:),1)));
    n4_lf_sin(i,:) = not(and3(circshift(X2_stream_lfsr_coeff(ceil((1/6)*N),:),1), n3_lf_sin(i,:), circshift(n1_lf_sin(i,:),2)));
    y_lf_sin(i,:) = and(n4_lf_sin(i,:), circshift(X2_stream_lfsr_input(i,:),6));
    sin_lfsr(i) = sum(y_lf_sin(i,:))/N;
    
    n1_lf_cos(i,:) = and(X2_stream_lfsr_input(i,:), circshift(X2_stream_lfsr_input(i,:),4));
    n2_lf_cos(i,:) = not(and(n1_lf_cos(i,:), circshift(X2_stream_lfsr_coeff(ceil((1/56)*N),:),1)));
    %n2_lf_cos(i,:) = n1_lf_cos(i,:);
    n3_lf_cos(i,:) = not(and3(circshift(X2_stream_lfsr_coeff(ceil((1/30)*N),:),1), n2_lf_cos(i,:), circshift(n1_lf_cos(i,:),1)));
    n4_lf_cos(i,:) = not(and3(circshift(X2_stream_lfsr_coeff(ceil((1/12)*N),:),1), n3_lf_cos(i,:), circshift(n1_lf_cos(i,:),2)));
    y_lf_cos(i,:) = not(and3(circshift(X2_stream_lfsr_coeff(ceil((1/2)*N),:),1), n4_lf_cos(i,:), circshift(n1_lf_cos(i,:),3)));
