...
for i = 1:N
    for k = 1:N
        ...
        if i/N > vd(k,4)%4 for sobol, 7 for vdc, 8 for LFSR --4
        %if i/N > sobol(k,2)
            X2_stream_vdc(i,k) = 1;
        end
        if i/N > vd(k,log2(N))%6 %5,4---10
        %if i/N > z1(k)
            X3_stream_vdc(i,k) = 1;
        end
        if i/N > vd(k,9)%9 
...
%     end
    ...
    n2_v(i,:) = not(and3(n1_v(i,:), circshift(X3_stream_vdc(ceil((1/4)*N),:),0), circshift(input,0)));
    n3_v(i,:) = not(and3(circshift(X5_stream_vdc(ceil((1/3)*N),:),0), n2_v(i,:), circshift(input,0)));
    n4_v(i,:) = not(and3(circshift(X4_stream_vdc(ceil((1/2)*N),:),0), n3_v(i,:), circshift(input,0)));
    y_v(i,:) = not(and(n4_v(i,:), circshift(input,0)));
...
%     y_lf(i,:) = and(n4_lf(i,:), circshift(X2_stream_lfsr(i,:),6));
    n1_lf(i,:) = not(and(X2_stream_lfsr_input(i,:), circshift(X2_stream_lfsr_coeff(uint32((1/5)*N),:),0)));
    n2_lf(i,:) = not(and3(n1_lf(i,:), circshift(X2_stream_lfsr_coeff(uint32((1/4)*N),:),0), circshift(X2_stream_lfsr_input(i,:),1)));
    n3_lf(i,:) = not(and3(circshift(X2_stream_lfsr_coeff(uint32((1/3)*N),:),0), n2_lf(i,:), circshift(X2_stream_lfsr_input(i,:),2)));
    n4_lf(i,:) = not(and3(circshift(X2_stream_lfsr_coeff(uint32((1/2)*N),:),1), n3_lf(i,:), circshift(X2_stream_lfsr_input(i,:),3)));
    y_lf(i,:) = not(and(n4_lf(i,:), circshift(X2_stream_lfsr_input(i,:),4)));
